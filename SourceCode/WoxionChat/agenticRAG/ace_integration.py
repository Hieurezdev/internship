...
class ACEIntegration:
    ...
    ACE_AVAILABLE: bool = ACE_AVAILABLE_FLAG

    @classmethod
    def get_user_playbook_collection(cls):
    ...
    @classmethod
    def get_user_failure_memories_collection(cls):
    ...
    @classmethod
    def load_playbook(cls, user_id: str, return_status: bool = False):
        """Load the ACE playbook for a specific user from MongoDB. Returns default if not found."""
        user_id = user_id or "default_user"
        was_created = False
        
        try:
            collection = cls.get_user_playbook_collection()
    ...
    @classmethod
    def save_playbook(cls, user_id: str, playbook_str: str) -> bool:
        ...
        try:
            ...
            if user_id in cls._retrievers:
                try:
                    cls._retrievers[user_id].update_index(playbook_str)
                    cls._playbook_hashes[user_id] = hashlib.md5(playbook_str.encode('utf-8')).hexdigest()
                    ...
                    cls._retrievers.pop(user_id, None)
                    cls._playbook_hashes.pop(user_id, None)
    ...
    @classmethod
    def get_ace_retriever(cls, user_id: str, top_k: int = 5) -> Optional[PlaybookRetriever]:
        """Get or initialize the user-specific PlaybookRetriever (RAE) from cache or MongoDB."""
        if not cls.ACE_AVAILABLE:
            return None

        user_id = user_id or "default_user"
        playbook_str = cls.load_playbook(user_id)
        ...
        if user_id not in cls._retrievers or cls._playbook_hashes.get(user_id) != current_hash:
            try:
                ...
                cls._retrievers[user_id] = retriever
                cls._playbook_hashes[user_id] = current_hash
            except Exception as e:
                logger.error(f"Failed to initialize ACE PlaybookRetriever for user {user_id}: {e}", exc_info=True)
                return None

        return cls._retrievers[user_id]

    @classmethod
    def get_ace_context(cls, query: str, user_id: str, top_k: int = 10) -> str:
        """
        ...
        try:
            retriever = cls.get_ace_retriever(user_id, top_k=top_k)
    ...
    @classmethod
    def load_failure_memory(cls, user_id: str, retriever: Optional[PlaybookRetriever] = None) -> FailureMemoryBank:
        """Load the FailureMemoryBank for a specific user from MongoDB, reusing the retriever's encoder."""
        user_id = user_id or "default_user"
        
        # Return from cache if already loaded
        if user_id in cls._failure_memories:
            return cls._failure_memories[user_id]
        ...
        try:
            collection = cls.get_user_failure_memories_collection()
        ...
        cls._failure_memories[user_id] = memory_bank
        return memory_bank
    ...
    def reflect_and_curate_feedback(
        ...
        try:
            ...
            retriever = cls.get_ace_retriever(user_id)
            bullets_used_str = ""
            if retriever:
            ...
            memory_bank = cls.load_failure_memory(user_id, retriever)
            ...
            save_success = cls.save_playbook(user_id, updated_playbook)
...
# Backward compatibility wrappers/aliases
ACE_AVAILABLE = ACEIntegration.ACE_AVAILABLE
DEFAULT_PLAYBOOK_CONTENT = DEFAULT_PLAYBOOK_CONTENT
get_user_playbook_collection = ACEIntegration.get_user_playbook_collection
get_user_failure_memories_collection = ACEIntegration.get_user_failure_memories_collection
load_playbook = ACEIntegration.load_playbook
save_playbook = ACEIntegration.save_playbook
get_ace_retriever = ACEIntegration.get_ace_retriever
get_ace_context = ACEIntegration.get_ace_context
load_failure_memory = ACEIntegration.load_failure_memory
get_openai_compatible_gemini_client = ACEIntegration.get_openai_compatible_gemini_client
reflect_and_curate_feedback = ACEIntegration.reflect_and_curate_feedback
parse_playbook_to_json = ACEIntegration.parse_playbook_to_json
